...
echo "编译主程序"
cd "$App_Path"
dotnet publish -c Release -p:PublishDir="$Publish_Path" -f net10.0-macos -p:CreatePackage=false -p:EnableCodeSigning=true -p:CodesignKey="$CodesignKey_Name"
...
dotnet publish -c Release -p:PublishDir="$Publish_Path" -f net10.0-macos
Copy_Plugins "$Plugins_Release_Path/net10.0-macos" "Accelerator" "BD.WTTS.Client.Plugins.Accelerator.dll"
...
if [ "$Build_Mode" = "any" ]; then
    dotnet publish -c Release -p:PublishDir="$Publish_Path/arm64" -f net10.0 -r osx-arm64 -p:PublishSingleFile=true --self-contained -p:EnableCodeSigning=true -p:CodesignKey="$CodesignKey_Name"
    dotnet publish -c Release -p:PublishDir="$Publish_Path/x64" -f net10.0 -r osx-x64 -p:PublishSingleFile=true --self-contained -p:EnableCodeSigning=true -p:CodesignKey="$CodesignKey_Name"
...
dotnet publish -c Release -p:PublishDir="$Publish_Path" -f net10.0-macos -p:EnableCodeSigning=true -p:CodesignKey="$CodesignKey_Name"
Copy_Plugins "$Plugins_Release_Path/net10.0-macos" "Authenticator" "BD.WTTS.Client.Plugins.Authenticator.dll"

echo "编译账户插件"
Plugins_GameAccount_Path="$Base_Path/src/BD.WTTS.Client.Plugins.GameAccount"
cd "$Plugins_GameAccount_Path"
dotnet publish -c Release -p:PublishDir="$Publish_Path" -f net10.0-macos -p:EnableCodeSigning=true -p:CodesignKey="$CodesignKey_Name"
Copy_Plugins "$Plugins_Release_Path/net10.0-macos" "GameAccount" "BD.WTTS.Client.Plugins.GameAccount.dll"

echo "编译游戏库插件"
Plugins_GameList_Path="$Base_Path/src/BD.WTTS.Client.Plugins.GameList"
cd "$Plugins_GameList_Path"
dotnet publish -c Release -p:PublishDir="$Publish_Path" -f net10.0-macos -p:EnableCodeSigning=true -p:CodesignKey="$CodesignKey_Name"
Copy_Plugins "$Plugins_Release_Path/net10.0-macos" "GameList" "BD.WTTS.Client.Plugins.GameList.dll"
...
# 根据 Build_Mode 执行不同逻辑
if [ "$Build_Mode" = "any" ]; then
    cp -r "$App_Modules_Path" "$Plugins_Release_Path/net10.0-macos/Steam++.app/Contents/MonoBundle"
elif [ "$Build_Mode" = "arm64" ]; then
    cp -r "$App_Modules_Path" "$Plugins_Release_Path/net10.0-macos/osx-arm64/Steam++.app/Contents/MonoBundle"
    cp -r "$Plugins_Release_Path/net10.0-macos/Steam++.app/Contents/Resources" "$Plugins_Release_Path/net10.0-macos/Steam++.app/Contents"
elif [ "$Build_Mode" = "x64" ]; then
    cp -r "$App_Modules_Path" "$Plugins_Release_Path/net10.0-macos/osx-x64/Steam++.app/Contents/MonoBundle"
    cp -r "$Plugins_Release_Path/net10.0-macos/Steam++.app/Contents/Resources" "$Plugins_Release_Path/net10.0-macos/Steam++.app/Contents"
fi

echo "代码签名重新手动签名"
App_Publish_Path="$App_Path/bin/Release/net10.0-macos/Steam++.app"
# #代码签名重新手动签名
