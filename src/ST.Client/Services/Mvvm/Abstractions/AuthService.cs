...
namespace System.Application.Services.Abstractions
{
    public abstract class AuthService : ReactiveObject, Interface
    {
        ...
        protected abstract MyAuthenticator GetMyAuthenticator();
        ...
        public async Task InitializeAsync(GameAccountPlatformAuthenticator[] auths, bool isSync = false)
        {
            ...
            {
                var authenticators = list.Select(s => GetMyAuthenticator(s));
        ...
        Run:
            ...
            if (result.resultCode == GAPRepository.ImportResultCode.Success)
            {
                ...
                {
                    AddOrUpdateSaveAuthenticators(GetMyAuthenticator(item), isLocal, password);
                }
        ...
        {
            AddOrUpdateSaveAuthenticators(GetMyAuthenticator(auth), isLocal, password);
        }
