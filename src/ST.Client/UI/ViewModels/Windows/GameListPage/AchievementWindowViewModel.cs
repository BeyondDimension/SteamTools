...
namespace System.Application.UI.ViewModels
{
    ...
    {
        public static string DisplayName => AppResources.Achievement_Manage;
        ...
        public AchievementWindowViewModel(int appid)
        {
            ...
            ISteamworksLocalApiService.Instance.AddUserStatsReceivedCallback((param) =>
            {
                if (param.Result != 1)
                {
                    MessageBox.ShowAsync(AppResources.Achievement_Warning_2.Format(param.Result), Title, MessageBox.Button.OK).ContinueWith(s =>
                    {
                        EnforceClose();
                    }).Wait();
                }
                ...
                {
                    MessageBox.ShowAsync("Failed to load schema", Title, MessageBox.Button.OK).ContinueWith(s =>
                    {
                        EnforceClose();
                    }).Wait();
                }
                ...
                Toast.Show(AppResources.Achievement_LoadSucces.Format(_AchievementsSourceList.Count, _StatisticsSourceList.Count));
            });
        ...
        int StoreAchievements()
        {
            ...
            foreach (AchievementInfo info in achievements)
            {
                ...
                {
                    MessageBox.ShowAsync(AppResources.Achievement_Warning_3.Format(info.Name), Title, MessageBox.Button.OK).ContinueWith(s =>
                    {
                        EnforceClose();
                    });
                    return -1;
        ...
        public async void ResetAllStats_Click()
        {
            ...
            if (result.IsOK())
            {
                var achievementsToo = await MessageBox.ShowAsync(AppResources.Achievement_ResetWaring_2, Title, MessageBox.Button.OKCancel);
                var real = await MessageBox.ShowAsync(AppResources.Achievement_ResetWaring_3, Title, MessageBox.Button.OKCancel);
                if (real.IsOK())
                {
        ...
        public void SaveChange_Click()
        {
            ...
            Toast.Show(AppResources.Achievement_EditSucces.Format(achievements, stats));
            RefreshStats_Click();
        }
    }
}