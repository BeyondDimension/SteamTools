...
public abstract class AuthMessageRecordRepository<TDbContext, TAuthMessageRecord, TSendSmsRequestType, TEAuthMessageType> : Repository<TDbContext, TAuthMessageRecord, Guid>, IAuthMessageRecordRepository<TAuthMessageRecord, TSendSmsRequestType, TEAuthMessageType>
    where TDbContext : DbContext
    where TAuthMessageRecord : AuthMessageRecord<TSendSmsRequestType, TEAuthMessageType>
    where TSendSmsRequestType : struct, Enum
    where TEAuthMessageType : struct, Enum
{
    public AuthMessageRecordRepository(TDbContext dbContext, IHttpContextAccessor httpContextAccessor) : base(dbContext, httpContextAccessor)
    {
    }

    protected abstract TEAuthMessageType GetDefaultEAuthMessageType();
    ...
    /// <returns></returns>
    protected abstract bool IsPhoneNumberOrEmail(TEAuthMessageType type);

    public async Task<TAuthMessageRecord?> GetMostRecentVerificationCodeWithoutChecksumAndMoDiscard(TEAuthMessageType type, string phoneNumberOrEmail, TSendSmsRequestType useType)
    {
    ...
    public async Task<TAuthMessageRecord?> CheckAuthMessageAsync(ISmsSender smsSender, string phoneNumberOrEmail, string message, TSendSmsRequestType useType, TEAuthMessageType? type = null)
    {
        var type_ = type ?? GetDefaultEAuthMessageType();
    ...
    public async Task<DateTimeOffset?> GetLastSendSmsTime(string phoneNumberOrEmail, TSendSmsRequestType? requestType = null, TEAuthMessageType? type = null)
    {
    ...
    public async Task<bool> IsMaxSendSmsDay(string phoneNum, byte? maxSendSmsDay = null, TEAuthMessageType? type = null)
    {
        var type_ = type ?? GetDefaultEAuthMessageType();
